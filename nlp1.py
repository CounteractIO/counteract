...
# returns a sourcefile's array of sentences, dictionary of words with respective counts, and arbitrary (but 
# consistently) ordered list of words 
...
def vectorize(sentence, words, word_list):
    ...
    for word in word_list:
        l.append(count(sentence.split(" "), word) * 1.0 / len(sentence.split(" ")) * 1 / words.get(word,
                                                                                                   1))  # adds vector data on relative frequency of word
    return l
...
print(
    "On a scale from 0-100, indicate the 'positivity' of the statement, where 100 represents an extremely positive "
    "statement.")

print(
    "You will have to hit enter to submit each time. You will also have to hit enter an additional time when all "
    "sentences have been prompted.")
...
for sentence in test_sent:
    test_d[sentence] = vectorize(sentence, words,
                                 word_list)  # we use the dictionary and list ordering from the training data for 
    # consistency 
